...
const Enquiry = () => {
    ...
    const getDataForTable = useCallback(async () => {
        const { data } = await AxiosHelper.getData("/admin/sushmagroup-enquiry-datatable", param);
        if (data?.status === true) {
                ...
                <table className="table table-bordered table-striped fs--1 mb-0">
                    <thead className="bg-200 text-900">
                        <tr>
                            ...
                            {records.some(row => row.projectName) && (
                                <th onClick={handelSort} className={`sort ${param?.orderBy === "projectName" && (param?.orderDirection === 1 ? 'asc' : 'desc')}`} data-sort="projectName">Site Visit Project Name</th>
                            )}
