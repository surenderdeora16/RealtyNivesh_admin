const {PinnacleEnquiry } = require('../../models');
...
async function getOtpStatusVerifiedAndNotRequiredCount() {
    const counts = await PinnacleEnquiry.countDocuments({
        otpStatus: { $in: ['otp verified', 'not required'] }
    });
    return counts;
}

exports.PinnacleEnquiry = async (req, res) => {
    try {
        ...
        if (action === 'getintouch') {
            const result = await PinnacleEnquiry.create({ ...data, otpStatus: 'not required' });
            const Emailcount = await getOtpStatusVerifiedAndNotRequiredCount();
            await sendEmail(data, Emailcount, 'The Pinnacle');

            return res.successInsert(result);

        } else if (action === 'submitForm') {
            const result = await PinnacleEnquiry.create({ ...data, otpStatus: 'otp not verified' });
            ...
            if (valid) {
                const enquiry = await PinnacleEnquiry.findOneAndUpdate(
                    { mobile },
            ...
            const enquiry = await PinnacleEnquiry.findOne({ mobile: data.mobile });
